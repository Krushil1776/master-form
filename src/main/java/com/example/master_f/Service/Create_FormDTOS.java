 ...
 * 
 * import com.example.master_f.Modal.AnswerType; import
 * com.example.master_f.Modal.Create_FormDTO; import
 * com.example.master_f.Modal.Form; import com.example.master_f.Modal.Question;
 * import com.example.master_f.Repositry.AnswerTypeR; import
 * com.example.master_f.Repositry.FormR; import
 * com.example.master_f.Repositry.QuestionR;
 * 
 * @Service public class Create_FormDTOS {
 * 
 * @Autowired AnswerTypeR answerTypeR;
 * 
 * @Autowired FormR formR;
 * 
 * @Autowired QuestionR questionR;
 * 
 ...
 * ResponseEntity.badRequest().body("Form data is missing"); }
 * 
 * Form existingForm = formR.findById(id).orElse(null); Form form;
 * 
 * if (existingForm != null) { form = existingForm; } else { form = new Form();
 * 
 * }
 * 
 * // Set form properties form.setTitle_text(formDTO.getTitle_text());
 ...
 * form.setActive(formDTO.getActive()); form.setText(formDTO.getText());
 ...
 * List<Question> questionDTOs = createDTO.getQuestion(); if (questionDTOs !=
 * null && !questionDTOs.isEmpty()) { List<Question> questions =
 * questionDTOs.stream().map(questionDTO -> { Question question; if
 * (questionDTO.getId() != null && questionDTO.getFormid()!=form.getId() ) {
 * question = questionR.findById(questionDTO.getId()).orElse(new Question());
 * System.out.println("AA"); } else { System.out.println("BB"); question = new
 * Question(); }
 * 
 * question.setqName(questionDTO.getqName());
 * question.setValidateq(questionDTO.getValidateq());
 ...
 * 
 * 
 * // Handle answer types List<AnswerType> answerTypesDTOs =
 * createDTO.getAnswerTypes(); if (answerTypesDTOs != null &&
 * !answerTypesDTOs.isEmpty()) { List<AnswerType> choices =
 * answerTypesDTOs.stream().map(answerTypeDTO -> { AnswerType choice =
 * answerTypeDTO.getId() != null ?
 * answerTypeR.findById(answerTypeDTO.getId()).orElse(new AnswerType()) : new
 * AnswerType();
 * 
 ...
 * 
 * return choice; }).collect(Collectors.toList());
 * 
 * // Save answer types answerTypeR.saveAll(choices); }
 * 
...
public class Create_FormDTOS {
    ...
    public ResponseEntity<String> saveDTO(Integer id, Create_FormDTO createDTO) {
//    	 Form f = formR.findById(id).orElse(null);
//
//    	    if (f != null) {
//    	        // Existing form
//    	        Form qForm = createDTO.getForms();
//    	        qForm.setModifieby((Integer) httpSession.getAttribute("userId"));
//    	        qForm.setModifieon(LocalDateTime.now());
//    	        formR.save(qForm);
//
//    	        List<Question> q = createDTO.getQuestion();
//
//    	        questionR.qdelete(qForm.getId());
//    	        System.out.println(qForm.getId());
//    	        questionR.saveAll(q);
//    	        answerTypeR.saveAll(createDTO.getAnswerTypes());
//
...
//    	        // New form
//    	        Form qForm = createDTO.getForms();
//    	        qForm.setCreateby((Integer) httpSession.getAttribute("userId"));
//    	        qForm.setCreateon(LocalDateTime.now());
//    	        formR.save(qForm);
//
//    	        questionR.saveAll(createDTO.getQuestion());
//    	        answerTypeR.saveAll(createDTO.getAnswerTypes());
//
...
//    }
        ...
        Form form = formR.findById(id).orElse(new Form());
        ...
        form.setCreateby((Integer)httpSession.getAttribute("userId"));
        ...
        if (questionDTOs != null && !questionDTOs.isEmpty()) {
            List<Question> questions = questionDTOs.stream().map(questionDTO -> {
                ...
                answerTypeR.deleteanswer(questionDTO.getQuestionlabel());
        ...
        if (answerTypesDTOs != null && !answerTypesDTOs.isEmpty()) {
            List<AnswerType> choices = answerTypesDTOs.stream().map(answerTypeDTO -> {
                AnswerType choice = answerTypeDTO.getId() != null ? 
                        answerTypeR.findById(answerTypeDTO.getId()).orElse(new AnswerType()) : new AnswerType();
